...
function SalesChart({ bookings, numDays }) {
  ...
  const data = allDates.map(date => {
    return {
      ...
      totalSales: bookings.filter(booking => isSameDay(date, new Date(booking.createdAt))).reduce((acc, cur) => acc + cur.totalPrice, 0),
      extrasSales: bookings.filter(booking => isSameDay(date, new Date(booking.createdAt))).reduce((acc, cur) => acc + cur.extraPrice, 0),
    };
  });
